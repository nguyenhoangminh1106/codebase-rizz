...
This file is the single source of truth for the allowlist. `bootstrap` writes these rules to `~/.claude/settings.json` during the cron install step, and `_shared/crons.md`'s plist template uses `--permission-mode bypassPermissions` to enforce them. If you add a new shell command to any skill, add it here first.
