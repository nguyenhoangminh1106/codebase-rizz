...
- If Gmail MCP is not detected: "I don't see a Gmail MCP in this session. Do you want to set one up? (y/n)"
  - If yes → print the install instructions from `../_shared/mcp-install.md` (section: Gmail). Ask the user to run the install and come back. Stop this subskill — the user re-runs it after installing. Don't try to half-finish
  - If no → mark Gmail channel as disabled, move on
...
The shape is defined in `../_shared/config-schema.md` under "Notifications".
...
Don't inline install instructions here — they go in `../_shared/mcp-install.md` so they can be maintained in one place. This subskill reads that file and relays the relevant section when a user needs to install something.
