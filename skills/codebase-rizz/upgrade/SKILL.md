...
3. **Determine which repo(s) to migrate.** For single-repo: resolve the current repo via `../references/paths.md` registry lookup. For all-repos: iterate every entry in `~/.codebase-rizz/registry.json`.
...
If a new cron key is being added and the user opts in (or the default is to enable it), generate the plist using the template in `../references/crons.md`, write it to `~/Library/LaunchAgents/com.codebase-rizz.<slug>.<cron-key>.plist`, and add the `launchctl load` command to the "run these after" list.
