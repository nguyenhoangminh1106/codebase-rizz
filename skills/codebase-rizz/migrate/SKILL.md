...
Resolve the current repo's registry entry via the lookup in `../references/paths.md`. If the lookup fails (repo never bootstrapped), tell the user to run `bootstrap` first. There's nothing to migrate if there's no data.
...
1. **Compute the target path.**
   ...
   - Repo-local → global: derive slug from remote URL per `../references/paths.md`, `target = ~/.codebase-rizz/repos/<slug>`
