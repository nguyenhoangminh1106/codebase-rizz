...
Default: daily at 6:00am. Configured via `crons.from_pr_comments` in `rizz.config.json`. Bootstrap translates this into a launchd user agent (`~/Library/LaunchAgents/com.codebase-rizz.from-pr-comments.plist`) and prints the `launchctl load` command for the user to run. See `../_shared/crons.md`.
